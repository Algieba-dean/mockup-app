# privacy-policy-generator Specification

## Purpose
Provide a standalone "隐私与条款" (Privacy & Terms) tool that walks a developer through generating a store-submission-ready Privacy Policy and/or Terms of Use entirely client-side (no network/AI calls), covering the same ground as external tools like `app-privacy-policy-generator.firebaseapp.com` (plus baseline sections that genre-standard generators include: Log Data, Do Not Track, International Data Transfers, Account/Data Deletion, and Terms-of-Use boilerplate like Eligibility/Indemnification/Severability), so developers never have to leave MockupApp to prepare compliance docs.
